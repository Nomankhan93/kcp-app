...
export function AdminWardCouncilors() {
  ...
  async function loadData() {
    ...
    try {
      const [wardRows, userRows] = await Promise.all([
        fetchWardCouncilorManagementRows(),
        fetchPortalUsersWithRoles(),
      ]);
      setRows(wardRows);
      ...
      setEdits(
        Object.fromEntries(
          wardRows.map((row) => [row.ward, makeEditable(row)]),
        ),
      );
    } catch (loadError) {
      setError(
        loadError instanceof Error
          ? loadError.message
          : "Unable to load ward councilor assignments. Run staff-ward-management-v1.sql first.",
      );
    } finally {
      setLoading(false);
    }
  ...
  function updateEdit(ward: string, values: Partial<EditableWardCouncilor>) {
    setEdits((current) => ({
      ...
      [ward]: {
        ...(current[ward] ?? {
          userId: "",
          fullName: `General Councilor ${ward}`,
          mobile: "",
    ...
    try {
      ...
      setError(
        saveError instanceof Error
          ? saveError.message
          : "Unable to save ward assignment.",
      );
    } finally {
      setSavingWard("");
    }
        ...
        {access.allowed ? (
          <>
            ...
            <div className="mt-6 rounded-3xl border border-blue-200 bg-blue-50 p-5 text-sm leading-6 text-blue-900">
              ...
              <p className="mt-1">
                Create the councilor login in Supabase Auth first, then select
                that user here. Saving assignment automatically gives the
                selected user the limited general_councilor role. This page is
                admin-only.
              </p>
            </div>
              ...
              <div className="mt-4 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <h2 className="text-lg font-black text-slate-950">
                    Ward assignment register
                  </h2>
                  <p className="text-sm text-slate-500">
                    One active General Councilor should be linked to each ward.
                  </p>
                </div>
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            Certificate applications from this ward go to this
                            councilor.
                          </p>
                        </div>
                        <span
                          className={`rounded-full px-2.5 py-1 text-xs font-bold ring-1 ${edit.isActive ? "bg-emerald-50 text-emerald-700 ring-emerald-100" : "bg-slate-50 text-slate-500 ring-slate-100"}`}
                        >
                      ...
                      <div className="mt-4 grid gap-2">
                        ...
                        <input
                          value={edit.mobile}
                          onChange={(event) =>
                            updateEdit(row.ward, { mobile: event.target.value })
                          }
                        ...
                        <select
                          value={edit.userId}
                          onChange={(event) =>
                            updateEdit(row.ward, { userId: event.target.value })
                          }
                          className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none ring-civic-600 transition focus:ring-2"
                        >
                      ...
                      <p className="mt-3 text-xs text-slate-500">
                        Current login:{" "}
                        {row.email ?? row.user_id ?? "Not assigned"}
                      </p>
                    </div>
                  );
                })}
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            ...
                            <p className="mt-2 text-xs text-slate-500">
                              {row.user_id
                                ? "Assigned user has ward access after saving."
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        ...
        {label === "Active Councilors" ? (
          <UserCog className="h-5 w-5 text-civic-700" />
        ) : (
