  ...
  const analytics = useMemo(() => {
    ...
    const averageResolutionDays = resolvedWithDates.length
      ? resolvedWithDates.reduce(
          (sum, item) =>
            sum + daysBetween(item.created_at, item.resolved_at as string),
          0,
  ...
  const calendarStats = useMemo(() => {
    return {
      today: complaints.filter((item) =>
        isInDateRange(item.created_at, "today"),
      ).length,
      week: complaints.filter((item) => isInDateRange(item.created_at, "7days"))
        .length,
      month: complaints.filter((item) =>
        isInDateRange(item.created_at, "month"),
      ).length,
    };
  }, [complaints]);
    ...
    try {
      ...
      setError(
        loadError instanceof Error
          ? loadError.message
          : "Unable to load chairman dashboard.",
      );
    } finally {
      setLoading(false);
    }
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        {loading && sessionState === "checking" ? (
          <div className="flex justify-center py-12 text-slate-500">
            <Loader2 className="mr-2 h-5 w-5 animate-spin" /> Checking chairman
            dashboard access...
          </div>
        ) : null}
        ...
        {sessionState === "signed-in" && !allowed ? (
          <div className="rounded-3xl border border-rose-200 bg-rose-50 p-6 text-rose-800">
            ...
            <p className="mt-2 text-sm">
              Chairman Dashboard is available only for accounts assigned as
              chairman or admin.
            </p>
          ...
          <>
            <div className="mb-6 flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-500">
                  Selected range: {rangeLabels[dateFilter]}. Data is calculated
                  from complaints currently accessible to this account.
                </p>
              </div>
            ...
            <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
              ...
              <SummaryCard
                ...
                helper={formatPercent(analytics.pendingRate)}
              />
              <SummaryCard
              ...
              <SummaryCard
                ...
                helper={formatPercent(analytics.resolutionRate)}
              />
              <SummaryCard
              ...
              <SummaryCard
                ...
                value={
                  analytics.averageResolutionDays
                    ? `${analytics.averageResolutionDays.toFixed(1)}d`
                    : "—"
                }
            ...
            <div className="mb-6 grid gap-4 lg:grid-cols-3">
              <ExecutiveFocus
                ...
                helper="New complaints registered today across all visible departments."
                tone={calendarStats.today ? "civic" : "slate"}
              />
              <ExecutiveFocus
                title="Oldest pending queue"
                value={longPending.length}
                helper="Review the long-pending list below for chairman follow-up."
                tone={longPending.length ? "amber" : "emerald"}
              />
              <ExecutiveFocus
                title="Priority follow-up"
                value={importantComplaints.length}
                helper="High or urgent pending complaints requiring office attention."
                tone={importantComplaints.length ? "rose" : "emerald"}
              />
            </div>
            ...
            <div className="mb-6 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
              <Panel
                ...
                description="Current status distribution for selected range."
              >
                  ...
                  {statusMetrics.map((item) => (
                    ...
                    >
                      ...
                      <ProgressBar
                        value={
                          analytics.total
                            ? (item.total / analytics.total) * 100
                            : 0
                        }
                      />
                    </div>
              ...
              <Panel
                ...
                description="Useful numbers for daily review."
              >
                <div className="space-y-3">
                  ...
                  />
                  <Indicator
                    label="Rejected / Not Related"
                    value={analytics.rejected + analytics.notRelated}
                    total={analytics.total}
                  />
                </div>
              </Panel>
            ...
            <div className="mb-6 grid gap-6 lg:grid-cols-2">
              <Panel
                ...
                description="Shows departments/categories with highest workload."
              >
                <MetricList
              ...
              <Panel
                title="Area / ward wise issues"
                description="Shows wards or areas with most complaints."
              >
                <MetricList
                  metrics={areaMetrics}
            ...
            <div className="grid gap-6 xl:grid-cols-3">
              ...
              <ComplaintList
                ...
                items={importantComplaints}
                emptyText="No high/urgent pending complaints."
              />
              <ComplaintList
                title="Recently resolved"
                items={recentlyResolved}
                emptyText="No resolved complaints in selected range."
              />
            </div>
          </>
        ) : null}
...
type ExecutiveTone = "slate" | "civic" | "amber" | "emerald" | "rose";

function executiveToneClasses(tone: ExecutiveTone) {
...
function ExecutiveFocus({
  ...
  return (
    <div
      className={`rounded-3xl border p-5 shadow-sm ${executiveToneClasses(tone)}`}
    >
...
function ProgressBar({ value }: { value: number }) {
  ...
  return (
    <div className="mt-3 h-2 overflow-hidden rounded-full bg-white ring-1 ring-slate-100">
      <div
        className="h-full rounded-full bg-civic-700"
